...
// ═══════════════════════════════════════════════════════════════════
// WEB CHAT ENDPOINT - Mobile-friendly chat page
// ═══════════════════════════════════════════════════════════════════
...
<head>
    ...
    <title>RangerBlock Chat v1.2.0</title>
    ...
    <div class="modal-overlay" id="connectModal">
        <div class="modal">
            <div class="modal-header">
                ...
                <div class="modal-subtitle">P2P Blockchain Network - Bridge Mode</div>
            </div>
            ...
            <div class="server-info">
                <div>Relay: ${RELAY_NAME} (${RELAY_REGION})</div>
        ...
        function handleMessage(data) {
            switch(data.type) {
                ...
                case 'machine_registered':
                    // Handle machine registry responses
                    if (data.machines) {
                        data.machines.forEach(m => {
                            if (m.nodeId !== nodeId) {
                                users.set(m.nodeId, { id: m.nodeId, name: m.name || m.nodeId, emoji: m.emoji || getEmoji(m.nodeId) });
                            }
                        });
                        updateUsers();
                    }
