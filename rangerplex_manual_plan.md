...
- **Stage 1 — Discovery & Gaps:** Confirm feature inventory against current UI; list missing sections/screenshots; align manual version with `package.json`; decide in-app placement (Sidebar vs Settings → Help).  
- **Stage 2 — In-App Mirror:** Build Manual page, render markdown, add anchor-based nav/search; add “Try it” prefill buttons for commands.  
